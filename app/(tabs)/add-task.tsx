...
export default function AddTaskScreen() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        // Simulate API call
        const response = await axios.post(`${config.url}/tasks`,
          {
            data: {
              title: values.title,
        ...
        Toast.show({
          text1: t('addTask.success.title'),
          text2: t('addTask.success.message'),
          type: 'success',
          visibilityTime: 4000,
        })
        ...
        Toast.show({
          text1: t('addTask.error.title'),
          text2: t('addTask.error.message'),
          type: 'error',
          visibilityTime: 4000,
        })
          ...
          <View className="px-6 py-4">
            ...
            <View className="mb-4">
              ...
              <TouchableOpacity
                ...
                className={`w-full px-4 py-4 border rounded-xl flex-row items-center justify-between ${formik.touched.type && formik.errors.type
                  ? 'border-red-500'
                  : (isDark ? 'border-gray-600 bg-gray-800' : 'border-gray-300 bg-white')
                  }`}
              >
                <Text className={`${formik.values.type
            ...
            <View className={`rounded-t-3xl p-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
              ...
              {taskTypes.map((type) => (
                <TouchableOpacity
                  ...
                  className={`py-4 px-4 rounded-xl mb-2 flex-row items-center justify-between ${formik.values.type === type.value
                    ? 'bg-blue-50 dark:bg-blue-900/20'
                    : (isDark ? 'bg-gray-700' : 'bg-gray-50')
                    }`}
                >
                  <Text className={`font-medium ${formik.values.type === type.value
                    ? 'text-blue-600'
                    : (isDark ? 'text-white' : 'text-gray-900')
                    }`}>
                    {type.label}
